...
class statistikController extends Controller
{
    public function gabungtable()
    {
		...
		$click = DB::SELECT(DB::raw("SELECT substr(peminjaman_ruangan.tanggal_peminjaman,1,4) as tahun, gedung.nama as gedung, COUNT(peminjaman_ruangan.tanggal_peminjaman) as jmlhpeminjam FROM ruangan JOIN peminjaman_ruangan ON ruangan.id = peminjaman_ruangan.ruangan_id JOIN gedung on ruangan.gedung_id=gedung.id GROUP BY ruangan.nama, YEAR(peminjaman_ruangan.tanggal_peminjaman)"));
	    $click = array_column($click, 'jmlhpeminjam');
	    
	    $viewer = DB::SELECT(DB::raw("SELECT substr(perbaikan_ruangan.tanggal_selesai_perbaikan,1,4) as tahun, gedung.nama as gedung, COUNT(perbaikan_ruangan.id) as jmlhperbaikan FROM ruangan JOIN perbaikan_ruangan ON ruangan.id = perbaikan_ruangan.ruangan_id JOIN gedung on ruangan.gedung_id=gedung.id GROUP BY ruangan.nama, YEAR(perbaikan_ruangan.tanggal_selesai_perbaikan)"));
